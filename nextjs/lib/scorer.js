...
for (const [canonical, aliases] of Object.entries(SKILL_SYNONYMS)) {
  ...
  for (const alias of aliases) SYNONYM_MAP[alias.toLowerCase()] = canonical;
}
...
function parseYear(dateStr) {
  ...
  if (lower.includes('present') || lower.includes('current') || lower.includes('now')) return new Date().getFullYear();
...
function isEducationalEntry(job) {
  return EDUCATION_INSTITUTION_RE.test(job.company || '') && EDUCATION_TITLE_RE.test(job.title || '');
}
...
function computeYearsScore(candidateYears, requiredYears) {
  ...
  if (candidateYears >= requiredYears) return Math.min(1.0, 0.8 + 0.2 * (candidateYears - requiredYears) / requiredYears);
...
function computeEducationScore(education, requiredDegree, requiredField) {
  ...
  let bestLevel = 0, bestField = null;
  for (const edu of education || []) {
    const level = getDegreeLevel(edu.degree);
    if (level !== null && level >= bestLevel) { bestLevel = level; bestField = edu.field; }
  }
  let score = bestLevel >= requiredLevel ? 1.0 : Math.max(0, 1.0 - (requiredLevel - bestLevel) * 0.3);
  if (requiredField && bestField) {
    const rF = requiredField.toLowerCase(), cF = bestField.toLowerCase();
...
function computeTitleScore(workExperience, jobTitle) {
  ...
  return Math.min(1.0, (0.6 * currentSim + 0.4 * maxHistorical) * 3);
}
...
function computeCertScore(resumeSkills, rawText, requiredCerts) {
  ...
  for (const cert of requiredCerts) {
    if (allSkillsText.includes(cert.toLowerCase()) || rawLower.includes(cert.toLowerCase())) matched++;
  }
...
function computeProjectScore(summary, workExperience, rawText) {
  ...
  if (allText.length > 500) score += 0.4;
...
export function getWeights(roleType, seniority, customWeights) {
  if (customWeights && typeof customWeights === 'object') {
    const vals = Object.values(customWeights);
    const isPercent = vals.some(v => v > 1);
    return Object.fromEntries(Object.entries(customWeights).map(([k, v]) => [k, isPercent ? v / 100 : v]));
  }
...
function passesHardFilters() { return true; }

export async function scoreResume(resumeId, jobProfileId) {
  ...
  if (!passesHardFilters()) return { overall: 0, band: 'Rejected', breakdown: {}, weights_used: {}, candidateYears: 0 };
...
export async function upsertScore(resumeId, jobProfileId) {
