...
export async function POST(request) {
  ...
  try {
    ...
    const summarySections = (topic.sections || []).filter(s => s.section_type === 'summary');
    if (summarySections.length === 0) {
      return NextResponse.json({ error: 'No summary section found for this topic' }, { status: 400 });
    }

    // All generated section headings become context
    const precedingSections = (topic.sections || [])
      .filter(s => s.is_generated && s.content && s.section_type !== 'summary')
      .map(s => s.heading);

    const updatedSections = [...(topic.sections || [])];
    const results = [];

    for (const sum of summarySections) {
      const idx = updatedSections.findIndex(s => s.id === sum.id);
      if (idx < 0) continue;
      ...
      results.push({ section_id: sum.id, content });
    }
