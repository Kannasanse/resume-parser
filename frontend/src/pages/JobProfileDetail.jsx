  ...
  return (
    <div className="relative overflow-hidden rounded-btn select-none">
      <button
        ...
        className="relative z-10 w-full px-5 py-2.5 text-sm font-semibold text-white bg-ds-danger rounded-btn transition-colors"
      >
        {progress > 0 ? `Hold to delete… ${remaining}s` : 'Hold to Delete'}
      </button>
      <div
        className="absolute inset-0 bg-red-800 rounded-btn transition-none pointer-events-none"
...
export default function JobProfileDetail() {
  ...
  const startEdit = () => {
    ...
    setEditWeights(job.custom_weights
      ? Object.fromEntries(Object.entries(job.custom_weights).map(([k, v]) => [k, v > 1 ? v : v * 100]))
      : { ...defaultW });
    setEditError('');
    setIsEditing(true);
  };
  ...
  const handleRoleTypeChange = (val) => {
    setEditRoleType(val);
    setEditWeights({ ...(WEIGHTS_TABLE[`${val}.${editSeniority}`] || WEIGHTS_TABLE['technical.mid']) });
  };
  const handleSeniorityChange = (val) => {
    setEditSeniority(val);
    setEditWeights({ ...(WEIGHTS_TABLE[`${editRoleType}.${val}`] || WEIGHTS_TABLE['technical.mid']) });
  };
  const updateWeight = (key, val) => {
    setEditWeights(prev => ({ ...prev, [key]: Math.max(0, Math.min(100, parseInt(val) || 0)) }));
  };

  const updateSkill = (i, field, value) =>
    setEditSkills(prev => prev.map((s, idx) => idx === i ? { ...s, [field]: value } : s));
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      await updateJob(id, {
        ...
        role_type: editRoleType, seniority: editSeniority,
        required_years_experience: parseInt(editYears) || 0,
        required_degree: editDegree,
        required_field: editField || null,
        required_certs: certsArray,
        custom_weights: editWeights,
        skills: editSkills,
      });
    ...
    return (
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <button onClick={() => setIsEditing(false)} className="text-sm text-ds-textMuted hover:text-ds-text transition-colors">
            ← Cancel
          </button>
          ...
          <div className="lg:col-span-3 space-y-5">
            ...
            <div className="bg-ds-card rounded border border-ds-border p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="number" min={0} max={30} value={editYears}
                    onChange={e => setEditYears(e.target.value)} className={inputCls} />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input type="text" value={editCerts} onChange={e => setEditCerts(e.target.value)}
            ...
            <div className="bg-ds-card rounded border border-ds-border p-5">
              ...
              <RichTextEditor value={editDescription} onChange={setEditDescription}
                placeholder="Enter the job description…" minHeight="220px" />
            </div>
          </div>
                    ...
                    {editSkills.map((s, i) => (
                      <div key={i} className="flex items-center gap-2 bg-ds-bg border border-ds-border rounded px-3 py-2">
                        ...
                        <button onClick={() => removeSkill(i)}
                          className="text-ds-textMuted hover:text-ds-danger text-lg leading-none flex-shrink-0">×</button>
                      </div>
                    ))}
                  </div>
                )
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={startEdit}
            className="text-sm border border-ds-border text-ds-text px-4 py-1.5 rounded-btn font-medium hover:bg-ds-bg transition-colors">
            Edit
          </button>
      ...
      <div className="bg-ds-card rounded border border-ds-border p-6">
        ...
        {job.description && (
          <div className="mt-4 text-sm text-ds-textSecondary border-t border-ds-border pt-4 leading-relaxed rich-content"
        ...
        <div className="space-y-3">
          ...
          {!candidatesLoading && candidates.length === 0 && (
            <div className="text-center py-12 bg-ds-card rounded border border-ds-border">
              ...
              <Link to={`/upload?jobId=${id}`} className="mt-3 inline-block text-sm text-primary hover:underline">
              ...
              <div key={c.resume_id} className="bg-ds-card rounded border border-ds-border overflow-hidden">
                <div className="flex items-center gap-4 p-4">
                  ...
                  <div className="flex-shrink-0">
                    {c.score ? <ScoreBreakdown score={c.score} compact /> : <span className="text-xs text-ds-textMuted">Not scored</span>}
                  </div>
