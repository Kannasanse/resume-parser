  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-xl p-5 space-y-4">
        <h2 className="text-sm font-semibold text-gray-700 uppercase tracking-wide">Scoring Parameters</h2>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          {/* Required years */}
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Required Years of Experience</label>
            <input
          ...
          {/* Required degree */}
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Minimum Degree</label>
            <select value={requiredDegree} onChange={e => setRequiredDegree(e.target.value)}
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-indigo-400">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Required Certifications <span className="text-gray-400">(comma-separated)</span></label>
            <input
              type="text"
              value={requiredCerts}
              onChange={e => setRequiredCerts(e.target.value)}
              placeholder="e.g. AWS SAA, CKA"
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400"
            />
          </div>
        </div>
          ...
          <div className="space-y-2">
            {skills.map((s, i) => (
              <div key={i} className="flex items-center gap-2 bg-white border border-gray-200 rounded-lg px-3 py-2">
                ...
                <button onClick={() => removeSkill(i)} className="text-gray-300 hover:text-red-400 text-lg leading-none">×</button>
              </div>
            ))}
          </div>
        </div>
